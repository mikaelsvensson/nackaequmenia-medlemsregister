    ...
    <?php foreach ($entries as $key => $entry) { ?>
        <div class="panel panel-default">
            <div class="panel-heading" role="tab" id="heading-<?= $key ?>">
                <?php
                $checked = isset($_POST['entry']) && in_array($key, $_POST['entry']);
                printf('<input type="checkbox" name="entry[]" value="%s" id="checkbox-%s" %s style="float: left; margin: 0.1em 1.0em 0em 0em">', $key, $key, $checked ? 'checked="checked"' : "");
                ?>
                <h4 class="panel-title" role="button" data-toggle="collapse" data-parent="#accordion"
                    ...
                    <?= $entry[name] ?>
                    <small style="white-space: nowrap"> <?= $entry[age] ?> år</small>
                </h4>
            </div>
